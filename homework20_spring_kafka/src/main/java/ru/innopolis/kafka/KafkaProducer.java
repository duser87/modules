...
public class KafkaProducer {
    ...
    public Flux<SenderResult<Void>> sendMessage(Integer count){
        return Flux.range(0, count)
                .map( i ->{
                    String message = "Сообщение № " + i + " для Kafka";
                    var kafkaMessage = KafkaMessage.builder()
                            .id(Long.valueOf(i))
                            .message(message)
                            .sendTime(LocalDateTime.now())
                            .build();
                    return MessageBuilder.withPayload(kafkaMessage).setHeader(MessageHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE).build();
                })
                .flatMap(kafkaMessage -> {
                    log.info("Отправка сообщения в Kafka: " + kafkaMessage.getPayload().getId());
